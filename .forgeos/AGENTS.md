...
1. .forgeos/instructions/core.instructions.md
2. .forgeos/instructions/*.instructions.md
3. .forgeos/agents/*.agent.md (includes Assigned Tool Loadout)
4. This file (.forgeos/AGENTS.md)
5. Delegation prompt
...
RULE: ALL agents operate under strict Tool Loadouts defined in their `.forgeos/agents/{Agent}.agent.md` file.
...
7. Read your agent file: .forgeos/agents/{YourAgent}.agent.md — internalize the Assigned Tool Loadout
8. Read upstream summary from .forgeos/agent-output/{PreviousAgent}/{ticket-id}.md (if exists)
9. Read .forgeos/vibecoding/chunks/{YourAgent}.agent/ (all files)
10. Read .forgeos/vibecoding/catalog.yml; load task-relevant chunks
11. Invoke `sequentialthinking/sequentialthinking` to plan execution before touching any files

## 2) Identity Invariants

- Ticketer is a **dumb dispatcher**: it never reads or writes codebase files. It ONLY evaluates ticket state via `.forgeos/tickets.py`, launches subagents with scoped toolsets via `runSubagent`, and performs claim/state commits. Its toolset is restricted to `memory/*`, `execute/*`, `github/*`, and `sequentialthinking/*`.
...
Before DONE, entry must exist in .forgeos/memory-bank/activeContext.md:
...
- .forgeos/instructions/core.instructions.md
- .forgeos/instructions/sdlc.instructions.md
- .forgeos/instructions/ticket-system.instructions.md
- .forgeos/instructions/git-protocol.instructions.md
- .forgeos/instructions/agent-behavior.instructions.md
- .forgeos/instructions/terminal-management.instructions.md
- .forgeos/tickets.py
