...
Run `python3 .forgeos/tickets.py --status --json` to see full ticket landscape.
...
1. Run `python3 .forgeos/tickets.py --sync` to release expired claims and fix state.
...
4. Run `python3 .forgeos/tickets.py --validate` to check integrity.

---

# STEP 4 — LEASE CLEANUP

1. Run `python3 .forgeos/tickets.py --release-expired` to clear all stale claims.
...
| File | Action |
|------|--------|
| `.forgeos/memory-bank/activeContext.md` | Append session summary: what was worked on, current state |
| `.forgeos/memory-bank/progress.md` | Append: tickets completed this session, completion percentage |
| `.forgeos/memory-bank/decisionLog.md` | Append: session decisions and trade-offs (Ticketer only) |
| `.forgeos/memory-bank/riskRegister.md` | Append: new risks identified, resolved risks |
| `.forgeos/memory-bank/feedback-log.md` | Append: QA/Validator/CI feedback from this session |

Then generate:

**`.forgeos/memory-bank/SESSION_SUMMARY.md`**
...
**`.forgeos/memory-bank/SYSTEM_SNAPSHOT.json`**
...
Run: `python3 .forgeos/tickets.py --status --json` to get current state.
Read `.forgeos/memory-bank/riskRegister.md` for open risks.

Generate:

**`.forgeos/memory-bank/RESUME_POINT.md`**
...
- `.forgeos/memory-bank/SESSION_SUMMARY.md`
- `.forgeos/memory-bank/SYSTEM_SNAPSHOT.json`
- `.forgeos/memory-bank/RESUME_POINT.md` (this file)
...
- Tickets completed: {n}
- Tickets remaining: {n}
- Resume point: .forgeos/memory-bank/RESUME_POINT.md
...
- All agents follow their Assigned Tool Loadout from `.forgeos/agents/{Agent}.agent.md`
