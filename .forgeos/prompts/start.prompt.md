...
3. The CTO agent file exists at `.forgeos/agents/CTO.agent.md`

---

## Phase 0 — Boot & Safety Check

1. Read `.forgeos/guardian/STOP_ALL` — if contains `STOP`: halt immediately.
2. Read all `.forgeos/instructions/*.instructions.md` (core, sdlc, ticket-system, git-protocol, agent-behavior).
3. Read `.forgeos/agents/CTO.agent.md` — internalize the CTO execution pipeline.
...
6. Run `python3 .forgeos/tickets.py --status --json` — confirm no existing tickets or understand current state.
...
3. **Tickets exist:** Run `python3 .forgeos/tickets.py --status --json` to confirm tickets in READY
4. **Integrity check:** Run `python3 .forgeos/tickets.py --validate` — zero errors
5. **Memory gate:** `.forgeos/memory-bank/activeContext.md` has CTO initialization entry
