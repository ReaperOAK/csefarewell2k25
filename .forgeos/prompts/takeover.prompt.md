...
2. Read all `.forgeos/instructions/*.instructions.md` (6 files).
3. Read your agent file: `.forgeos/agents/{YourAgent}.agent.md` — internalize the Assigned Tool Loadout.
4. Run `python3 .forgeos/tickets.py --sync`
5. Run `python3 .forgeos/tickets.py --status --json`
6. Read `.forgeos/skills/{YourAgent}/` (all files).
7. Read `.forgeos/vibecoding/catalog.yml`; load task-relevant chunks.
8. Invoke `sequentialthinking` to plan execution before touching any files.

All agents operate under strict Tool Loadouts per `.forgeos/AGENTS.md` §0.1. No tool browsing or hallucination outside assigned loadout.
...
5. Parse L3 tasks into ticket JSON:
   ```bash
   python3 .forgeos/tickets.py --parse TODO/tasks/
   ```
...
7. Verify ticket state:
   ```bash
   python3 .forgeos/tickets.py --status
   ```
...
- Write agent summary to `.forgeos/agent-output/{AgentName}/{ticket-id}.md`
...
- `python3 .forgeos/tickets.py --validate` passes integrity check

Then continue normal autonomous execution:
- Ticket by ticket via `python3 .forgeos/tickets.py --sync` + `--status --json`
...
- All agents follow their Assigned Tool Loadout from `.forgeos/agents/{Agent}.agent.md` — no out-of-scope tool usage
- Parallelized across operators/machines with push-based distributed locking
- Full SDLC loop through stage directories
- Agent summary handoff via `.forgeos/agent-output/{AgentName}/{ticket-id}.md`
...
- Memory bank updates per `.forgeos/instructions/core.instructions.md`
...
```bash
python3 .forgeos/tickets.py --status
python3 .forgeos/tickets.py --validate
```
