...
```
read_file(".forgeos/memory-bank/RESUME_POINT.md")
read_file(".forgeos/memory-bank/SESSION_SUMMARY.md")
read_file(".forgeos/memory-bank/SYSTEM_SNAPSHOT.json")
```
...
```
read_file(".forgeos/memory-bank/activeContext.md")
read_file(".forgeos/memory-bank/progress.md")
read_file(".forgeos/memory-bank/decisionLog.md")
read_file(".forgeos/memory-bank/riskRegister.md")
run_in_terminal("python3 .forgeos/tickets.py --status --json")
```

**1c.** Get current ticket landscape:

```bash
python3 .forgeos/tickets.py --sync
python3 .forgeos/tickets.py --status --json
```

**1d.** Verify boot sequence compliance — ensure agents follow the 11-step boot from `.forgeos/AGENTS.md`:
...
```bash
mkdir -p .forgeos/memory-bank/archive
mv .forgeos/memory-bank/RESUME_POINT.md .forgeos/memory-bank/archive/RESUME_POINT-{date}.md
mv .forgeos/memory-bank/SESSION_SUMMARY.md .forgeos/memory-bank/archive/SESSION_SUMMARY-{date}.md
mv .forgeos/memory-bank/SYSTEM_SNAPSHOT.json .forgeos/memory-bank/archive/SYSTEM_SNAPSHOT-{date}.json
```
...
No skipping stages. No partial execution. Each stage advances via `.forgeos/tickets.py --advance`.
...
1. Re-run `python3 .forgeos/tickets.py --sync` to evaluate dependencies and move newly unblocked tickets to READY.
2. Re-run `python3 .forgeos/tickets.py --status --json`.
...
> Execute `.forgeos/prompts/stop.prompt.md`
...
- All agents read their own chunks from `.forgeos/skills/{Agent}/`
- All agents derive context from filesystem — Ticketer does NOT inject context
- All agents follow their Assigned Tool Loadout defined in `.forgeos/agents/{Agent}.agent.md` — no tool browsing or hallucination outside assigned loadout
