...
# Allow .forgeos/agent-output summary writes
if echo "$INPUT" | grep -qE '.forgeos/agent-output/.*\.md'; then
    exit 0
fi

# Allow memory-bank writes
if echo "$INPUT" | grep -qE '\.forgeos/memory-bank/'; then
    exit 0
fi

# Block everything else
echo "TICKETER SCOPE VIOLATION: Ticketer is a dispatcher, not an implementer." >&2
echo "  Ticketer may only modify: tickets/*.json, ticket-state/**/*.json, .forgeos/agent-output/**/*.md, memory-bank/*.md" >&2
