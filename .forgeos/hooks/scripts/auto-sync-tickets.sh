...
# Purpose: Run .forgeos/tickets.py --sync before any agent session work begins
# Exit 0 = allow session to proceed, Exit 1 = surface error but don't block

set -euo pipefail

TICKETS_SCRIPT=".forgeos/tickets.py"
...
# Check .forgeos/tickets.py exists
...
if python3 "$TICKETS_SCRIPT" --sync 2>&1; then
    ...
    echo "WARNING: Ticket sync encountered errors. Check .forgeos/tickets.py output above." >&2
fi
