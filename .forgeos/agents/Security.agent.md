...
1. Read `.forgeos/guardian/STOP_ALL` — if it contains `STOP`, halt immediately, zero edits.
2. Read all `.forgeos/instructions/*.instructions.md` (core, sdlc, ticket-system, git-protocol, agent-behavior, terminal-management).
3. Read upstream QA summary from `.forgeos/agent-output/QA/{ticket-id}.md`.
4. Read all chunks in `.forgeos/skills/Security/`.
...
→ Reject ticket. Execute: `python3 .forgeos/tickets.py --rework {ticket-id} Security "{finding summary}"`
→ Append entry to `.forgeos/memory-bank/riskRegister.md` with threat details, severity, and recommended fix.

## 7. Work Commit (Commit 2)

1. Write security report to `.forgeos/agent-output/Security/{ticket-id}.md` including: STRIDE model, OWASP checklist, SARIF findings, SBOM summary, verdict.
2. Delete previous stage summary: `.forgeos/agent-output/QA/{ticket-id}.md`.
3. If PASS: move ticket to `ticket-state/CI/{ticket-id}.json`.
4. If FAIL: rework via `.forgeos/tickets.py` (ticket stays in SECURITY or returns to its implementation stage).
5. Append memory entry to `.forgeos/memory-bank/activeContext.md`:
   ```
   ### [{ticket-id}] — Security Review
   - **Artifacts:** .forgeos/agent-output/Security/{ticket-id}.md
   - **Decisions:** {verdict} — {rationale}
   - **Timestamp:** {ISO8601}
   ```
6. Update `.forgeos/memory-bank/riskRegister.md` if new risks identified.
...
- [.forgeos/instructions/*.instructions.md](../.forgeos/instructions/*.instructions.md) — canonical system rules.
- [.forgeos/skills/Security/](../.forgeos/skills/Security/) — detailed patterns, code examples, checklists.
