...
3. Read upstream summary from `.forgeos/agent-output/Documentation/{ticket-id}.md`.
4. Read `.forgeos/skills/Validator/` (all chunk files).
...
| 10 | Memory gate entry exists | Verify `[TICKET-ID]` block exists in `.forgeos/memory-bank/activeContext.md` |
...
## 6. Verdict Actions
- **APPROVE:** `python3 .forgeos/tickets.py --advance {ticket-id} Validator` → move to DONE.
- **REJECT:** `python3 .forgeos/tickets.py --rework {ticket-id} Validator "{reason}"` → back to implementation stage with evidence.

## 7. Work Commit (Commit 2)
1. Write validation report to `.forgeos/agent-output/Validator/{ticket-id}.md`.
...
4. If REJECTED: ticket goes back for rework (.forgeos/tickets.py handles state).
5. Run `python3 .forgeos/tickets.py --sync` to unblock freed downstream tasks.
6. Write memory entry to `.forgeos/memory-bank/activeContext.md`:
   ```
...
## 11. References
- [.forgeos/instructions/*.instructions.md](../.forgeos/instructions/*.instructions.md) (all 6 canonical files)
- [.forgeos/skills/Validator/](../.forgeos/skills/Validator/) (chunk-01, chunk-02, chunk-03)
