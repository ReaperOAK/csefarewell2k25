...
1. Read `.forgeos/guardian/STOP_ALL` — if contains `STOP`: zero edits, report blocked.
2. Read all `.forgeos/instructions/*.instructions.md` (core, sdlc, ticket-system, git-protocol, agent-behavior, terminal-management).
3. Read upstream summary from `.forgeos/agent-output/Security/{ticket-id}.md`.
4. Read all files in `.forgeos/skills/CIReviewer/`.
...
- **FAIL** → reject with SARIF evidence:
  ```bash
  python3 .forgeos/tickets.py --rework {ticket-id} CIReviewer "{reason with finding summary}"
  ```

## 7. Work Commit (Commit 2)

1. Write CI report to `.forgeos/agent-output/CIReviewer/{ticket-id}.md` containing:
   verdict, quality score, SARIF findings summary, metrics per file.
2. Delete upstream summary: `rm .forgeos/agent-output/Security/{ticket-id}.md`.
3. If PASS: move ticket JSON to `ticket-state/DOCS/{ticket-id}.json`.
   If FAIL: ticket stays for rework processing (.forgeos/tickets.py handles move).
4. Append memory entry to `.forgeos/memory-bank/activeContext.md`:
   ```markdown
   ### [{ticket-id}] — CI Review
   - **Artifacts:** .forgeos/agent-output/CIReviewer/{ticket-id}.md
   - **Decisions:** {verdict} — Score {N}/100, {N} critical, {N} warnings
   - **Timestamp:** {ISO8601}
   ```
5. Stage ONLY modified files explicitly — **NEVER `git add .`**:
   ```bash
   git add .forgeos/agent-output/CIReviewer/{ticket-id}.md
   git add ticket-state/DOCS/{ticket-id}.json   # or CI/ if rework
   git add tickets/{ticket-id}.json
   git add .forgeos/memory-bank/activeContext.md
...
| SARIF report | Generated at `.forgeos/agent-output/CIReviewer/{ticket-id}.sarif` |
...
- [.forgeos/skills/CIReviewer/](../.forgeos/skills/CIReviewer/)
