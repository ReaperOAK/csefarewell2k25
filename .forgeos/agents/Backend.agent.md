...
1. Read `.forgeos/guardian/STOP_ALL` — if contains `STOP`: halt immediately, zero edits.
2. Read all `.forgeos/instructions/*.instructions.md` (core, sdlc, ticket-system, git-protocol, agent-behavior, terminal-management).
3. Read upstream summary from `.forgeos/agent-output/{PreviousAgent}/{ticket-id}.md` (if exists).
4. Read all chunk files in `.forgeos/skills/Backend/`.
...
1. Write summary to `.forgeos/agent-output/Backend/{ticket-id}.md` including:
...
6. Stage ONLY modified files explicitly — one `git add <file>` per file:
   ...
   git add .forgeos/agent-output/Backend/{ticket-id}.md
...
- [.forgeos/skills/Backend/](../.forgeos/skills/Backend/)
- [.forgeos/vibecoding/catalog.yml](../.forgeos/vibecoding/catalog.yml)
