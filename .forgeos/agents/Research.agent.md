...
2. Read all files in `.forgeos/instructions/` (core, sdlc, ticket-system, git-protocol, agent-behavior, terminal-management)
3. Read upstream summary from `.forgeos/agent-output/{PreviousAgent}/{ticket-id}.md` (if exists)
4. Read all files in `.forgeos/skills/Research/`
...
4. Append memory entry to `.forgeos/memory-bank/activeContext.md`:
   ```markdown
   ### [{ticket-id}] — Summary
   - **Artifacts:** .forgeos/agent-output/Research/{ticket-id}.md
...
- [.forgeos/skills/Research/](../.forgeos/skills/Research/) (chunk-01.yaml, chunk-02.yaml)
