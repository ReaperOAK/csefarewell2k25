...
1. Read `.forgeos/guardian/STOP_ALL` — if contains `STOP`: halt, zero edits, report blocked
2. Read all `.forgeos/instructions/*.instructions.md` (core, sdlc, ticket-system, git-protocol, agent-behavior, terminal-management)
3. Read upstream summary from `.forgeos/agent-output/{PreviousAgent}/{ticket-id}.md`
4. Read all files in `.forgeos/skills/QA/`
...
- Advance ticket: `python3 .forgeos/tickets.py --advance {ticket-id} QA`

**FAIL** — Any gate fails:
- Document specific failures: file, line, test name, expected vs actual
- Send for rework: `python3 .forgeos/tickets.py --rework {ticket-id} QA "{reason}"`
- Rework reason must include actionable fix guidance

## 7. Work Commit (Commit 2)

1. Write QA report to `.forgeos/agent-output/QA/{ticket-id}.md` (include verdict, evidence, metrics)
2. Delete previous stage summary from `.forgeos/agent-output/{PreviousAgent}/{ticket-id}.md`
3. If PASS: move ticket to `ticket-state/SECURITY/{ticket-id}.json`
4. If FAIL: ticket stays in rework state (handled by .forgeos/tickets.py)
5. Update master copy at `tickets/{ticket-id}.json`
6. Append memory entry to `.forgeos/memory-bank/activeContext.md` with ticket-id, artifacts, verdict, mutation score, coverage, and ISO8601 timestamp
...
- **Included:** test files, test configs, test fixtures, coverage reports, QA reports, `.forgeos/agent-output/QA/`
...
- [.forgeos/instructions/*.instructions.md](../.forgeos/instructions/*.instructions.md) (core, sdlc, ticket-system, git-protocol, agent-behavior, terminal-management)
- [.forgeos/skills/QA/](../.forgeos/skills/QA/) (test strategy details, examples, report templates)
