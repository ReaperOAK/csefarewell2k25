...
4. Read `.forgeos/skills/UIDesigner/` (all chunks)
5. Read `.forgeos/vibecoding/catalog.yml` — load task-relevant chunks
6. Read ticket JSON from `ticket-state/` or `tickets/`
7. Read Stitch project ID from `.forgeos/stitch-project-id.txt` if exists (persist across tickets for continuity)
...
- Create Stitch project, (`stitch/create_project`) — only one for the entire project, persist project ID in memory for all subsequent calls at .forgeos/stitch-project-id.txt
...
1. Write summary to `.forgeos/agent-output/UIDesigner/{ticket-id}.md`
...
6. Append memory entry to `.forgeos/memory-bank/activeContext.md`:
   ```markdown
   ### [{ticket-id}] — Summary
...
## 10. References
- [.forgeos/instructions/*.instructions.md](../.forgeos/instructions/*.instructions.md) (all 6 canonical instruction files)
- [.forgeos/skills/UIDesigner/](../.forgeos/skills/UIDesigner/) (chunk-01, chunk-02)
