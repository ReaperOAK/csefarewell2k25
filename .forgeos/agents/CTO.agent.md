...
handoffs:
  - label: 'Launch TODO Decomposition'
    agent: 'CTO'
    prompt: 'Project initialization complete. Run TODO agent (L1>L2>L3) to decompose the plan into actionable tickets. Use `python3 .forgeos/tickets.py --parse TODO/tasks/` to parse the TODO output, then `python3 .forgeos/tickets.py --sync` to sync with the ticket system. Once tickets are in READY state, hand off to Ticketer for execution.'
    send: false
  - label: 'Execute Ticket Backlog'
    agent: 'Ticketer'
    prompt: '/continue Scan READY tickets and begin dispatching workers. Run python3 .forgeos/tickets.py --sync first, then dispatch one subagent per READY ticket following the SDLC pipeline.'
    send: false
---
...
| `execute/*` & `vscode/*` | Terminal commands, `.forgeos/tickets.py`, git operations |
...
1. Read `.forgeos/guardian/STOP_ALL` — if contains `STOP`: halt, zero edits.
2. Read all `.forgeos/instructions/*.instructions.md` (core, sdlc, ticket-system, git-protocol, agent-behavior).
3. Read `.forgeos/skills/CTO/` (all files, if exists).
4. Read `.forgeos/vibecoding/catalog.yml` — load task-relevant chunks.
...
7. Read `.forgeos/memory-bank/activeContext.md` and `.forgeos/memory-bank/progress.md` for prior context.
...
runSubagent("Research Analyst", prompt="
  ...
  Write summary to: .forgeos/agent-output/Research/CTO-research.md
")
```
...
runSubagent("ProductManager", prompt="
  ...
  - Research findings: .forgeos/agent-output/Research/CTO-research.md
  ...
  Write summary to: .forgeos/agent-output/ProductManager/CTO-prd.md
")
```
...
runSubagent("Architect", prompt="
  ...
  Context:
  - PRD: docs/PRD.md
  - Research: .forgeos/agent-output/Research/CTO-research.md
  ...
  Write summary to: .forgeos/agent-output/Architect/CTO-architecture.md
")
```
...
runSubagent("TODO", prompt="
  ...
  Parse tickets: python3 .forgeos/tickets.py --parse TODO/tasks/
  Sync state: python3 .forgeos/tickets.py --sync
  
  Write summary to: .forgeos/agent-output/TODO/CTO-decomposition.md
")
```
...
1. Run `python3 .forgeos/tickets.py --status --json` — verify tickets exist in READY.
2. Run `python3 .forgeos/tickets.py --validate` — verify ticket integrity.
3. Update `.forgeos/memory-bank/activeContext.md` with CTO session summary:
   ```markdown
...
- [.forgeos/instructions/core.instructions.md](../.forgeos/instructions/core.instructions.md)
- [.forgeos/instructions/sdlc.instructions.md](../.forgeos/instructions/sdlc.instructions.md)
- [.forgeos/instructions/ticket-system.instructions.md](../.forgeos/instructions/ticket-system.instructions.md)
- [.forgeos/instructions/git-protocol.instructions.md](../.forgeos/instructions/git-protocol.instructions.md)
- [.forgeos/instructions/agent-behavior.instructions.md](../.forgeos/instructions/agent-behavior.instructions.md)
- `.forgeos/tickets.py`
- `.forgeos/AGENTS.md`
