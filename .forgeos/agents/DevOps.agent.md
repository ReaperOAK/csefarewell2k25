...
1. Read `.forgeos/guardian/STOP_ALL` — if `STOP`: halt, zero edits, report blocked
...
4. Read `.forgeos/skills/DevOps/` (all chunk files)
...
1. Write summary to `.forgeos/agent-output/DevOps/{ticket-id}.md`
2. Delete previous stage summary (`.forgeos/agent-output/{PreviousAgent}/{ticket-id}.md`)
3. Move ticket JSON to next stage: `ticket-state/QA/{ticket-id}.json`
4. Update `.forgeos/memory-bank/activeContext.md` with entry:
...
- [.forgeos/skills/DevOps/](../.forgeos/skills/DevOps/)
