...
| `execute/*` | Terminal commands for `.forgeos/tickets.py`, git operations, and claim commits |
...
3. **Sync Tickets:** Use `execute/runInTerminal` to run `python3 .forgeos/tickets.py --sync` and `--status --json`.
...
1. Read `.forgeos/guardian/STOP_ALL` — if contains `STOP`: halt immediately, zero edits, zero dispatches.
2. Read all `.forgeos/instructions/*.instructions.md` (core, sdlc, ticket-system, git-protocol, agent-behavior).
3. Run `python3 .forgeos/tickets.py --sync` — releases expired claims, evaluates deps, moves unblocked to READY.
4. Run `python3 .forgeos/tickets.py --status --json` — get machine-readable state of all tickets.
...
5. On subagent completion: verify summary written to `.forgeos/agent-output/{Agent}/{ticket-id}.md`.
6. Advance ticket to next stage via `python3 .forgeos/tickets.py --advance <id> <agent>`.
7. Re-run `python3 .forgeos/tickets.py --sync` and repeat.
...
Ticketer does NOT skip stages. Ticketer does NOT reorder stages. Ticketer does NOT reason about dependencies — `.forgeos/tickets.py` handles all dependency resolution.
...
- NEVER reason about dependency graphs (`.forgeos/tickets.py` handles this)
...
- `.forgeos/tickets.py` — ticket state machine manager
