...
2. **Scan Local Instructions**: Discover existing instruction files in `.forgeos/instructions/` folder
...
12. **Download/Update Assets**: For requested instructions, automatically:
    - Download new instructions to `.forgeos/instructions/` folder
...
- **Path-specific instructions**: `.forgeos/instructions/NAME.instructions.md` (applies to specific file patterns via `applyTo` frontmatter)
...
- Scan local file system for existing instructions in `.forgeos/instructions/` directory
...
5. Preserve file location in `.forgeos/instructions/` directory
