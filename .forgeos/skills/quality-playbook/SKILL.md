...
| `.forgeos/AGENTS.md` | Bootstrap context for any AI session working on this project | The "read this first" file. Without it, AI sessions waste their first hour figuring out what's going on. | No |
...
**Find the specifications.** Specs are the source of truth for functional tests. Search in order: `.forgeos/AGENTS.md`/`CLAUDE.md` in root, `specs/`, `docs/`, `spec/`, `design/`, `architecture/`, `adr/`, then `.md` files in root. Record the paths.
...
### File 6: `.forgeos/AGENTS.md`

If `.forgeos/AGENTS.md` already exists, update it — don't replace it. Add a Quality Docs section pointing to all generated files.
...
| .forgeos/AGENTS.md | AI session bootstrap | Updated | ████████ High — factual |
```
