...
2. **Scan Local Custom Agents**: Discover existing custom agent files in `.forgeos/agents/` folder
...
11. **Download/Update Assets**: For requested agents, automatically:
    - Download new agents to `.forgeos/agents/` folder
...
1. List all `*.agent.md` files in `.forgeos/agents/` directory
...
- Scan local file system for existing agents in `.forgeos/agents/` directory
...
5. Preserve file location in `.forgeos/agents/` directory
