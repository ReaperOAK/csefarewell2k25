...
RULE: .forgeos/tickets.py enforces dependency resolution and stage transitions.

## 2. Rule Precedence

RULE: Apply first match only, highest wins:
1. `.forgeos/instructions/*.instructions.md`
2. `.forgeos/agents/*.agent.md`
3. Delegation packet
...
REQUIRED: Every agent reads `.forgeos/guardian/STOP_ALL` before any work.
...
1. `.forgeos/guardian/STOP_ALL`
2. `.forgeos/instructions/` (all 5 files)
3. `.forgeos/skills/` (load relevant skill SKILL.md and references)
4. `.forgeos/vibecoding/catalog.yml` (load task-relevant chunks)
5. Upstream summary from `.forgeos/agent-output/{PreviousAgent}/{ticket-id}.md`
...
REQUIRED: Before DONE, ticket must have entry in `.forgeos/memory-bank/activeContext.md`:
```markdown
### [TICKET-ID] — Summary
