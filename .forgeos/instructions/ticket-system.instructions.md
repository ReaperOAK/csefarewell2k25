...
name: Ticket System
applyTo: '**'
description: Ticket state machine, stage directories, .forgeos/tickets.py rules, dependency resolution, ticket JSON requirements, parallelism rules. Defines the file-based state machine for ticket management.
---
...
## 3. .forgeos/tickets.py Contract

RULE: Location: `.forgeos/tickets.py`
...
PROHIBITED: Any other agent executing .forgeos/tickets.py.
...
RULE: .forgeos/tickets.py evaluates dependencies, not agents.
RULE: No agent may manually move tickets to READY.
PROHIBITED: Agents reasoning about dependencies. .forgeos/tickets.py handles this.
